...
class GemmTestNew : public ::testing::Test {
  protected:
...
TYPED_TEST_SUITE(GemmTestNew, AllTestConfigs);

TYPED_TEST(GemmTestNew, gemm) {
...
TYPED_TEST(GemmTestNew, gemmNeg) {
...
TYPED_TEST(GemmTestNew, gemmAdd) {
...
TYPED_TEST(GemmTestNew, gemmSub) {
...
TYPED_TEST(GemmTestNew, gemmSubShiftA) {
    using batmat::linalg::gemm_sub;
    const auto ε = 1000 * std::numeric_limits<typename TestFixture::value_type>::epsilon();
    for (auto m : batmat::tests::sizes)
        for (auto n : batmat::tests::sizes)
...
TYPED_TEST(GemmTestNew, gemmSubShiftCD) {
    using batmat::linalg::gemm_sub;
    const auto ε = 1000 * std::numeric_limits<typename TestFixture::value_type>::epsilon();
    for (auto m : batmat::tests::sizes)
        for (auto n : batmat::tests::sizes)
...
TYPED_TEST(GemmTestNew, gemmSubShiftCDNeg) {
    using batmat::linalg::gemm_sub;
    const auto ε = 1000 * std::numeric_limits<typename TestFixture::value_type>::epsilon();
    for (auto m : batmat::tests::sizes)
        for (auto n : batmat::tests::sizes)
