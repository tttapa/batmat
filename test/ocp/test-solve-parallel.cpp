// TODO: integrate this in test-solve.cpp
...
TEST(OCP, solve) {
    ...
    auto ocp = ko::generate_random_ocp({.N_horiz = 40, //
                                        .nx      = 90,
                                        .nu      = 3,
                                        .ny      = 3,
                                        .ny_N    = 0},
    ...
    real_t S = 0.000001; // primal regularization
    ...
    // Perform the factorization of the KKT system (with wrong active set).
    {
        ...
        std::printf("Start OMP\n");
        auto t0 = std::chrono::steady_clock::now();
        for (int i = 0; i < 500; ++i)
            s.factor(S, Σ_strided, J0_strided);
        auto t1 = std::chrono::steady_clock::now();
        std::printf("Done OMP: %llu\n",
                    (unsigned long long)
                        duration_cast<std::chrono::microseconds>(t1 - t0)
                            .count());
    }
    {
        s.factor_tbb(S, Σ_strided, J0_strided);
        std::printf("Start TBB\n");
        auto t0 = std::chrono::steady_clock::now();
        for (int i = 0; i < 500; ++i)
            s.factor_tbb(S, Σ_strided, J0_strided);
        auto t1 = std::chrono::steady_clock::now();
        std::printf("Done TBB: %llu\n",
                    (unsigned long long)
                        duration_cast<std::chrono::microseconds>(t1 - t0)
                            .count());
    }
    {
        s.factor_fork(S, Σ_strided, J0_strided);
        std::printf("Start fork\n");
        auto t0 = std::chrono::steady_clock::now();
        for (int i = 0; i < 500; ++i)
            s.factor_fork(S, Σ_strided, J0_strided);
        auto t1 = std::chrono::steady_clock::now();
        std::printf("Done fork: %llu\n",
                    (unsigned long long)
                        duration_cast<std::chrono::microseconds>(t1 - t0)
                            .count());
    }
    {
        ...
        auto t0 = std::chrono::steady_clock::now();
        for (int i = 0; i < 500; ++i)
            s.factor_omp(S, Σ_strided, J0_strided);
        auto t1 = std::chrono::steady_clock::now();
        std::printf("Done omp: %llu\n",
                    (unsigned long long)
                        duration_cast<std::chrono::microseconds>(t1 - t0)
