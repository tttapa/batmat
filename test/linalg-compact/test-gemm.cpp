            ...
            for (index_t j = 0; j < C.rows(); ++j)
                for (index_t k = 0; k < C.cols(); ++k)
                    ASSERT_NEAR(C(i, j, k), C_reference(i, j, k), ε)
                        << enum_name(backend) << ": GEMM mismatch at (" << i
                        << ", " << j << ", " << k << ")";
    }
...
TYPED_TEST_SUITE(GemmTest, Abis);
