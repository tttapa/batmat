...
using koqkatoo::linalg::compact::DefaultStride;
...
TYPED_TEST_SUITE_P(BatchedMatrixViewParamTest);

TYPED_TEST_P(BatchedMatrixViewParamTest, TransposeMatchesOriginal) {
...
TYPED_TEST_P(BatchedMatrixViewParamTest, TransposeMatchesOriginalBlock) {
    const auto bs    = this->batch_size;
    const auto depth = bs * 3;
    auto trans =
        this->view.block(1, 2, 3, 4).middle_layers(bs, depth).transposed();
...
REGISTER_TYPED_TEST_SUITE_P(BatchedMatrixViewParamTest,
                            TransposeMatchesOriginal,
                            TransposeMatchesOriginalBlock);

using TestConfigs =
    ::testing::Types<ViewTypeConfig<1, true>, ViewTypeConfig<1, false>,
                     ViewTypeConfig<4, true>, ViewTypeConfig<4, false>>;

INSTANTIATE_TYPED_TEST_SUITE_P(BatchedMatrixViewTest,
                               BatchedMatrixViewParamTest, TestConfigs);
