...
void dsyrk_blasfeo(benchmark::State &state) {
    ...
    std::vector<real_t> A(n_elem), C(n_elem);
    ...
    auto batch_dsyrk_blasfeo = [&] {
#if KOQKATOO_WITH_OPENMP
        if (omp_get_max_threads() == 1) {
            for (index_t i = 0; i < depth; ++i)
                blasfeo_dsyrk_ln(n, n, 1.0, batch_A[i].get(), 0, 0,
                                 batch_A[i].get(), 0, 0, 1.0, batch_C[i].get(),
                                 0, 0, batch_C[i].get(), 0, 0);
            return;
        }
#endif
        ...
        for (index_t i = 0; i < depth; ++i)
            blasfeo_dsyrk_ln(n, n, 1.0, batch_A[i].get(), 0, 0,
                             batch_A[i].get(), 0, 0, 1.0, batch_C[i].get(), 0,
                             0, batch_C[i].get(), 0, 0);
    };
    for (auto _ : state) {
        batch_dsyrk_blasfeo();
    ...
    auto flop_cnt                 = 64e-9 * nd * (nd + 1) * nd / 2;
...
#define BM_RANGES()                                                            \
    DenseRange(1, 63, 1)                                                       \
        ...
        ->UseRealTime()
BENCHMARK(dsyrk_blasfeo)->BM_RANGES();
