...
void dtrsm_blasfeo(benchmark::State &state) {
    ...
    auto batch_dtrsm_blasfeo = [&] {
#if KOQKATOO_WITH_OPENMP
        if (omp_get_max_threads() == 1) {
            for (index_t i = 0; i < depth; ++i)
                blasfeo_dtrsm_llnn(n, n, 1, batch_A[i].get(), 0, 0,
                                   batch_B[i].get(), 0, 0, batch_B[i].get(), 0,
                                   0);
            return;
        }
#endif
        ...
        for (index_t i = 0; i < depth; ++i)
            blasfeo_dtrsm_llnn(n, n, 1, batch_A[i].get(), 0, 0,
                               batch_B[i].get(), 0, 0, batch_B[i].get(), 0, 0);
    };
    for (auto _ : state) {
        batch_dtrsm_blasfeo();
    }
...
#define BM_RANGES()                                                            \
    DenseRange(1, 63, 1)                                                       \
        ...
        ->UseRealTime()
BENCHMARK(dtrsm_blasfeo)->BM_RANGES();
