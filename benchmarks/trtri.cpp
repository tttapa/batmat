...
void trtri(benchmark::State &state) {
    ...
    for (auto _ : state)
        for (index_t l = 0; l < A.num_batches(); ++l)
            trtri(tril(A.batch(l)), tril(B.batch(l)));
...
BENCHMARK(trtri<default_abi, RowMajor, ColMajor>)->BM_RANGES();
BENCHMARK(trtri<default_abi, RowMajor, RowMajor>)->BM_RANGES();
BENCHMARK(trtri<default_abi, ColMajor, ColMajor>)->BM_RANGES();
BENCHMARK(trtri<default_abi, ColMajor, RowMajor>)->BM_RANGES();
