...
/// Generalized matrix multiplication D = C ± A⁽ᵀ⁾ B⁽ᵀ⁾ computing only the lower
/// triangle of C. Single register block.
template <class Abi, KernelConfig Conf, index_t RowsReg>
[[gnu::hot]] void
xgemmt_copy_microkernel(const single_batch_matrix_accessor<Abi, Conf.trans_A> A,
                        const single_batch_matrix_accessor<Abi, Conf.trans_B> B,
                        const single_batch_matrix_accessor<Abi> C,
                        const mut_single_batch_matrix_accessor<Abi> D,
    ...
    KOQKATOO_FULLY_UNROLLED_FOR (index_t jj = 0; jj < ColsReg; ++jj)
        KOQKATOO_FULLY_UNROLLED_FOR (index_t ii = jj; ii < RowsReg; ++ii)
            D_cached.template store<S>(rotr<S>(C_reg[ii][jj]), ii, jj);
}
...
void xgemmt_register(single_batch_view<Abi> A, single_batch_view<Abi> B,
    ...
    if (I <= RowsReg && I == J)
        return microkernel_t[I - 1](A_, B_, C_, K, init_zero);
    ...
    for (j = 0; j + ColsReg <= J; j += ColsReg) {
        ...
        for (index_t i = j; i < I; i += RowsReg) {
            ...
            if (i == j) {
                ...
                microkernel_t[nj - 1](Ai, Bj, Cij, K, init_zero);
            } else {
...
/// Generalized matrix multiplication D = C ± A⁽ᵀ⁾ B⁽ᵀ⁾ computing only the lower
/// triangle of D. Using register blocking.
template <class Abi, KernelConfig Conf>
void xgemmt_copy_register(single_batch_view<Abi> A, single_batch_view<Abi> B,
    ...
    const index_t I = C.rows(), J = C.cols();
    const index_t K = Conf.trans_A ? A.rows() : A.cols();
    KOQKATOO_ASSUME(I == D.rows());
    KOQKATOO_ASSUME(J == D.cols());
    KOQKATOO_ASSUME(I == (Conf.trans_A ? A.cols() : A.rows()));
    KOQKATOO_ASSUME(J == (Conf.trans_B ? B.rows() : B.cols()));
    KOQKATOO_ASSUME(I > 0);
    KOQKATOO_ASSUME(J > 0);
    KOQKATOO_ASSUME(K > 0);
    static const auto microkernel_t = microkernel_t_copy_lut<Abi, Conf>;
    static const auto microkernel   = microkernel_copy_lut<Abi, Conf>;
    const single_batch_matrix_accessor<Abi, Conf.trans_A> A_ = A;
    const single_batch_matrix_accessor<Abi, Conf.trans_B> B_ = B;
    ...
    // Optimization for very small matrices
    if (I <= RowsReg && I == J)
        return microkernel_t[I - 1](A_, B_, C_, D_, K);
    // Simply loop over all blocks in the given matrices.
    index_t j;
    for (j = 0; j + ColsReg <= J; j += ColsReg) {
        const auto nj = ColsReg;
        ...
        for (index_t i = j; i < I; i += RowsReg) {
            ...
            if (i == j) {
                KOQKATOO_ASSUME(ni == nj);
                microkernel_t[nj - 1](Ai, Bj, Cij, Dij, K);
            } else {
