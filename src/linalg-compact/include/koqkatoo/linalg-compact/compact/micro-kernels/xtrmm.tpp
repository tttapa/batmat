...
/// Generalized matrix multiplication C = C ± A B⁽ᵀ⁾, with B⁽ᵀ⁾ lower
/// trapezoidal.
...
/// Generalized matrix multiplication C = C ± A B⁽ᵀ⁾, with A upper
/// trapezoidal.
/// Single register block.
template <class Abi, KernelConfig Conf, index_t RowsReg, index_t ColsReg>
[[gnu::hot]] void
xtrmm_lunn_microkernel(const single_batch_matrix_accessor<Abi, Conf.trans_A> A,
    ...
    KOQKATOO_FULLY_UNROLLED_FOR (index_t l = 0; l < RowsReg; ++l)
        KOQKATOO_FULLY_UNROLLED_FOR (index_t ii = 0; ii <= l; ++ii) {
            simd Ail = A.load(ii, l);
            KOQKATOO_FULLY_UNROLLED_FOR (index_t jj = 0; jj < ColsReg; ++jj) {
                simd &Cij = C_reg[ii][jj];
                simd Blj  = B.load(l, jj);
