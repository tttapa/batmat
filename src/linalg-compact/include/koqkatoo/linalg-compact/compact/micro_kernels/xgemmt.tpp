...
namespace koqkatoo::linalg::compact::micro_kernels::gemmt {

using namespace koqkatoo::linalg::compact::micro_kernels::gemm;

/// C = C ± A⁽ᵀ⁾ B⁽ᵀ⁾, but accessing only the lower triangle of C.
template <class simd, KernelConfig Conf, index_t RowsReg>
void xgemmt_RxR_microkernel(auto A, auto B, auto C) {
...
void xgemmt_register(auto A, auto B, auto C) {
#ifdef __AVX512F__
    ...
    static constinit auto diag_microkernel_lut =
        make_1d_lut<RowsReg>([]<index_t R>(index_constant<R>) -> microkernel_t {
            return xgemmt_RxR_microkernel<simd, Conf, R + 1>;
        });
