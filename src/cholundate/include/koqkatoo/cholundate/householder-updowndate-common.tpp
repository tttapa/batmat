...
inline void tile_tail(index_t rowsA, index_t colsA, mut_W_accessor<> W,
                      real_t *L, index_t ldL, const real_t *B, index_t ldB,
                      real_t *A, index_t ldA, UpDown updown) noexcept {
    ...
    if constexpr (skip_large_M || skip_suboptimal_M) {
        if constexpr (sizeof...(Ms) > 0)
            tile_tail<Func, Conf, UpDown, Ms...>(rowsA, colsA, W, L, ldL, B,
                                                 ldB, A, ldA, updown);
        return;
    }
    while (rowsA >= M) {
        constexpr Config NewConf{.block_size_r = Conf.block_size_r,
                                 .block_size_s = M};
        Func<NewConf, UpDown>{}(colsA, W, L, ldL, B, ldB, A, ldA, updown);
        L += M;
        A += M;
        rowsA -= M;
    }
...
inline void
updowndate_tile_tail(index_t rowsA, index_t colsA, detail::mut_W_accessor<> W,
...
template <micro_kernels::householder::Config Conf, class UpDown>
inline void updowndate_tail(index_t colsA, detail::mut_W_accessor<> W,
                            detail::mut_matrix_accessor L,
                            detail::matrix_accessor B,
                            detail::mut_matrix_accessor A, UpDown signs) {
    using micro_kernels::householder::updowndate_tail;
    updowndate_tail<Conf, UpDown>(colsA, W, L.data, L.outer_stride, B.data,
