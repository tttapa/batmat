...
function(codegen_cholundate_microkernels tgt headers)
    ...
    foreach(alg "householder-updowndate")
        ...
        target_precompile_headers(${tgt}-impl-${alg} PRIVATE "${TPP_DIR}/../${alg}-serial.tpp")
        ...
        foreach(R 32 16 12 8 4 2 1)
            if (R GREATER KOQKATOO_MAX_SHH_KERNEL_WIDTH)
                continue()
            endif()
            ...
            foreach(S 32 24 16 12 8 4 2 1)
                if (S GREATER KOQKATOO_MAX_SHH_KERNEL_HEIGHT)
                    continue()
                endif()
    ...
    if (KOQKATOO_WITH_NAIVE_PARALLEL)
        add_library(${tgt}-impl-householder-updowndate-naive STATIC)
        target_link_libraries(${tgt}-impl-householder-updowndate-naive PRIVATE ${headers} warnings)
        target_precompile_headers(${tgt}-impl-householder-updowndate-naive PRIVATE "${TPP_DIR}/../householder-updowndate-naive.tpp")
        foreach(R 32 16 12 8 4 2 1)
            if (R GREATER KOQKATOO_MAX_SHH_KERNEL_WIDTH)
                continue()
            endif()
            foreach(S 32 24 16 12 8 4 2 1)
                ...
                math(EXPR S_R_REM "${S} % ${R}")
                if (S_R_REM EQUAL 0)
                    configure_file("${CMAKE_CURRENT_FUNCTION_LIST_DIR}/../householder-updowndate-naive.cpp.in" "${OUT_DIR}/householder-updowndate-naive-${R}-${S}.cpp" @ONLY)
                    koqkatoo_coroutine_flags("${OUT_DIR}/householder-updowndate-naive-${R}-${S}.cpp")
                    target_sources(${tgt}-impl-householder-updowndate-naive PRIVATE "${OUT_DIR}/householder-updowndate-naive-${R}-${S}.cpp")
                endif()
            endforeach()
        endforeach()
        foreach (op "diag" "full" "tail")
            target_link_libraries(${tgt}-impl-householder-updowndate-naive PRIVATE ${tgt}-microkernels-householder-updowndate-${op})
        endforeach()
        target_link_libraries(${tgt}-microkernels INTERFACE ${tgt}-impl-householder-updowndate-naive)
        list(APPEND KOQKATOO_CODEGEN_TARGETS ${tgt}-impl-householder-updowndate-naive)
    endif()

    if (KOQKATOO_WITH_STATIC_PARALLEL)
