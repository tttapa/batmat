    ...
    scalar_real_matrix D{{.depth = 1, .rows = (N + 1) * ny, .cols = 1}};
    {
        ...
        auto process_subdiag_block_W = [&](index_t k, auto Ad, auto Dd, auto Ld,
            ...
            foreach_chunked_merged(
                k + R, Ld.rows, S, [&](index_t i, auto rem_i) {
                    ...
                    microkernel_tail_lut[R - 1](rem_i, colsA, W, Lds, Ads, Add,
                                                DownUpdate{Dd});
                });
        };
        auto process_subdiag_block_V = [&](index_t k, index_t rem_k, auto As,
            ...
            foreach_chunked_merged(0, Ls.rows, S, [&](index_t i, auto rem_i) {
                ...
                microkernel_tail_lut[rem_k - 1](rem_i, colsA, W, Lss, Ass, Add,
                                                DownUpdate{Dd});
            });
        };
        ...
        for (index_t k = 0; k < N; ++k) {
            ...
            std::ranges::transform(Dd_spn, Dd_spn.begin(), std::negate<>{});
...
void Solver<Abi>::updowndate(real_view Σ, bool_view J_old, bool_view J_new) {
    ...
    for (index_t k = 0; k <= N; ++k) {
        ...
        for (index_t r = 0; r < nyk; ++r) {
            ...
            if (up || down) {
                auto s = storage.Σ_sgn(k, rj, 0) = up ? +real_t(0) : -real_t(0);
                storage.Σ_ud(k, rj, 0)           = cneg(Σ(k, rj, 0), s);
    ...
    foreach_chunked_merged(0, N + 1, simd_stride, [&](index_t k, auto nk) {
        ...
        index_t rj_batch = *rjmm.first, rj_min = *rjmm.second;
        auto Σj   = Σ.batch(batch_idx).top_rows(rj_batch);
        auto Σudj = storage.Σ_ud.batch(batch_idx).top_rows(rj_batch);
...
#ifdef _GLIBCXX_EXPERIMENTAL_SIMD
        ...
        for (index_t rj = rj_min; rj < rj_batch; ++rj) {
            ...
            where(z0_mask.__cvt(), zero).copy_to(&Σudj(0, 0, rj), vec_align);
        }
#else
#error "Fallback not yet implemented"
        ...
        // Copy Z
        compact_blas::xcopy(Zj, Z1j);
        // Z ← L⁻¹ Z
        compact_blas::xtrsm_LLNN(LH().batch(batch_idx), Zj, be);
        // Update LH
        compact_blas::xshhud_diag(LH().batch(batch_idx), Zj, Σudj, be);
        // Lu ← ZᵀZ ± Σ⁻¹
